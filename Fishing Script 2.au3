...
Func MainMacroFunction()
    While $running And Not $paused
        ...
        Send("1")
        Sleep(100)  ; Small delay for the game to register the input

        GUICtrlSetData($statusLabel, "Casting")
        MouseClick("left")  ; Perform a left mouse click
        Sleep(2000)  ; Small delay after clicking

        GUICtrlSetData($statusLabel, "Performing Action 'A'")
        Send("a")  ; Press 'a'
        Sleep(1000)  ; Small delay after pressing 'a'

        GUICtrlSetData($statusLabel, "Performing Action 'D'")
        Send("d")  ; Press 'd'
        Sleep(1000)  ; Wait 1 second before repeating the cycle
